...
\item No new algorithm, and no improvement on the neural-network literature for this problem.
The search is~\citeref{liwang2022} unchanged. Mańdziuk~\citeref{mandziuk1995} solved $n$-queens
thirty years ago with a binary Hopfield-type network --- the same $\{0,1\}$ state space we use ---
in both synchronous and asynchronous modes, and reports convergence rates up to $100\%$ with
polynomial average cost. Our solver does considerably worse than that, as \secref{sec:meas} shows.
The difference is that his network is designed for $n$-queens, whereas ours is a general reduction
to a network we had already formalized for other reasons, and the reduction is proved correct. What
is new here is the proof, not the performance.
