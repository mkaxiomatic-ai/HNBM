...
/-!
## Quiver Hamiltonian as a vector-parameter Gibbs energy

Shows that `HopfieldEnergy.hamiltonian` equals the abstract `VectorGibbs.energy` built from
`stat` and `thetaFromParams`, so BM learning and Quiver Gibbs dynamics use the same energy.
-/
...
open InnerProductSpace Matrix TwoState HopfieldEnergy

variable {U : Type} [Fintype U] [DecidableEq U] [Nonempty U]

/-- Coordinate-wise sufficient-statistic coefficients (before `WithLp.toLp`). -/
noncomputable def statCoeffs (s : State ℝ U) : I U → ℝ
  | Sum.inl (i, j) => (1 / 2 : ℝ) * s.act i * s.act j
  | Sum.inr i => -s.act i

/-- Coordinate-wise parameter coefficients (before `WithLp.toLp`). -/
...
/-- Inner product of sufficient statistic and parameter vector, expanded on coordinates. -/
lemma inner_stat_theta (p : Params ℝ U) (s : State ℝ U) :
    inner ℝ (stat s) (thetaFromParams p) =
      (1 / 2 : ℝ) * ∑ i, s.act i * (p.w.mulVec s.act i) -
        ∑ i, (p.θ i).get fin0 * s.act i := by
  classical
  have hW :
  ...
  have hθ :
      (∑ i : U, (-s.act i) * (p.θ i).get fin0) = -∑ i, s.act i * (p.θ i).get fin0 := by
...
/-- Abstract vector-parameter energy built from `stat` and `thetaFromParams`. -/
...
theorem vectorGibbsEnergy_eq_neg_inner (p : Params ℝ U) (s : State ℝ U) :
