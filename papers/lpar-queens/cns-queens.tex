...
maps every statement of this article to its Lean identifier. \secref{sec:related} places the
formulation among the neural, the QUBO and the verified-encoding literature.
...
\emph{3) Collaborative search.} A population of $K$ such networks is run concurrently
--- \citeref{liwang2022} writes $N$, which here is the board size. The best state
...
population collapses onto $x^{*}$. Termination is a run of more than $M$ consecutive rounds without
improvement to $x^{*}$.
...
formulas presume $N \ge 2$, as every instance below does; the development truncates $2N-3$ at zero, so
a $1\times1$ board carries one variable and two constraints rather than the counts~\eqref{eq:sizes}
returns. Otherwise they are confirmed on every row of \tabref{tab:results} and
\tabref{tab:blocked}.
...
\begin{qcpic}{0.62cm}
  ...
  \qcmark{1}{2}
  % labels sit outside the board, each at the outward end of the capsule it names
  \node[cRow,  font=\scriptsize] at (-0.45,-1.5) {$r_{1}$};   % row 1 runs to y = -1.5
  \node[cDiag, font=\scriptsize] at (1.5,0.34)   {$r_{9}$};   % diagonal ends at (0,1), x = 1.5
  \node[cCol,  font=\scriptsize] at (2.5,0.34)   {$r_{6}$};   % column 2, x = 2.5
  \node[cAnti, font=\scriptsize] at (3.5,0.34)   {$r_{15}$};  % anti-diagonal ends at (0,3), x = 3.5
\end{qcpic}
&
\begin{qcpic}{0.62cm}
  \qcboard{4}
  ...
  % the absent anti-diagonal points up and left off the board, so its label goes there
...
auxiliary variable lies in exactly one row. And a square carrying a given lies in one row more than
it otherwise would, namely its own constraint~\eqref{eq:form-giv}. Hence
\begin{equation}\label{eq:degs}
  \deg(u) \in \{1,3,4,5\}, \qquad \theta_u \in \{-\tfrac12,-\tfrac32,-2,-\tfrac52\},
\end{equation}
the odd degrees being what make $\theta$ half-integral, and $5$ occurring only at an interior square
that carries a given. \figref{fig:inc} draws an empty board, where the degrees are just $4$, $3$ and
$1$; on \texttt{Comp8} of \tabref{tab:results}, whose givens include both an interior square and a
corner, all four values occur.
...
additively. The $2(2N-3)$ constraints~\eqref{eq:form-diag} and~\eqref{eq:form-anti} carry a single
index $e$, the first $2N-3$ values being the diagonals and the remainder the anti-diagonals, and the
square $(i,j)$ lies on constraint $e$ exactly when
\begin{equation}\label{eq:onslack}
  i + N = e + 2 + j \quad\text{if } e < 2N-3, \qquad\qquad
  i + j + (2N-3) = e + 1 \quad\text{otherwise.}
\end{equation}
Both are equivalent to the intended condition over $\mathbb{Z}$, and neither ever subtracts. The
second reads oddly because $e$ is the index into the whole block rather than into the anti-diagonals
alone; with a local index it would be $i + j = e + 1$. The same hazard appears in the checker, where
it is discharged by \thmref{thm:spec}.
...
column degree of $\hat A$ & uniformly $4$ &
  $4$ at an interior square, $3$ at a corner, $1$ at an auxiliary variable, and one more at a square
  carrying a given\\
threshold & $\theta = -2\cdot\mathbf{1}$, uniform &
  $\theta_u \in \{-\tfrac12, -\tfrac32, -2, -\tfrac52\}$, non-uniform and half-integral at odd
  degrees\\
variable reduction & Algorithm~1; leaves between $0$ and $209$ of $729$ variables, and solves five
...
four constraints, so $\theta$ is the constant $-2$. Here it is $1$, $3$, $4$ or $5$
by~\eqref{eq:degs}, and $\theta_u$ correspondingly $-\tfrac12$, $-\tfrac32$, $-2$ or $-\tfrac52$.
...
\subsection{What the minimizers are}\label{sec:min}

\begin{thm}[the minimizers]\label{thm:min}
For the queens instance, the minimum energy is attained if and only if the instance has a completion;
and every state attaining it decodes to a completion.
\end{thm}

The second half does not reverse, and it is worth saying why. Decoding reads only the square
variables, not the auxiliary variables of~\eqref{eq:form-diag} and~\eqref{eq:form-anti}, so a state
whose board decodes to a completion need not be a minimizer. On the empty $4\times4$ board, flipping
one auxiliary bit of the encoded completion $q = (1,3,0,2)$ leaves the decoded board equal to $q$
while raising $p$ from $0$ to $1$. What \thmref{thm:iff} and \thmref{thm:min} give is therefore an
equivalence between the existence of a zero and the existence of a completion, together with
soundness of each individual minimizer --- which is what a search needs, since a search reports the
state it stopped at.

Combining \thmref{thm:net} with the uniqueness result of \secref{sec:hnbm} gives further that, for an
instance that has a completion, the stationary distribution of the Boltzmann machine at temperature
$T$ assigns vanishing probability to the infeasible states as $T \to 0^{+}$. The hypothesis is not
decoration: on a blocked instance no state attains $-\tfrac12\lVert\hat b\rVert_2^2$ and there is
nothing for the mass to concentrate on. The vanishing set is the states with $p > 0$, which by the
previous paragraph is strictly larger than the set of states whose boards are not completions. This is
a statement about the equilibrium distribution, not about any cooling schedule.

\subsection{Boards with no completion}\label{sec:blocked}

\begin{figure}[t]
\centering
% Two rows: five boards side by side would be about 16cm wide, which no reasonable \textwidth takes.
...
\end{qcpic}
\\[0.5em]
\qclab{(c) \texttt{Blk4}} & \qclab{(d)} & \qclab{(e)}
\end{tabular}
...
net input in the inner loop is proved equal to twice the network's local field --- net input less
threshold --- the factor of two being the one that lets $\theta$ be stored as $2\theta$; and the
minimizers are characterized by \thmref{thm:min}. The \emph{trajectory} is not certified, for the reason given in
...
\{\text{DHNm}~\eqref{eq:dhnm},\ \text{BMm}~\eqref{eq:bmm}\}$; and $K$, $M$, $c_0$, $c_1$, $c_2$,
...
p(\hat x^{(i)})$\\
\emph{2--7} \quad\, \textbf{repeat:} run each of the $K$ copies of $\mathcal{M}$ from $x^{(i)}(0)$ to
...
to its structure. A population of $K$ copies of DHNm \eqref{eq:dhnm} or BMm \eqref{eq:bmm} is run to
...
\bottomrule
\end{tabular}
\caption{The parameters of \figref{fig:alg}, as used for every \emph{collaborative} run in
\tabref{tab:results} and \tabref{tab:blocked}. \emph{Not one of these values is stated
in~\citeref{liwang2022}}; all were
recovered experimentally, and $c_2$ and $\mathcal{T}$ are not merely a matter of
...
that on the empty $8\times8$ board DHNm succeeds on four of ten seeds at BMm's $0.4$ against nine of
ten at its own $0.9$.
...
--- under the same hypothesis as \thmref{thm:iff}, that the givens lie on the board ---
...
constraint counts $n$ and $m$ of~\eqref{eq:sizes}; whether the checker accepts the board backtracking
returned; the total number of completions; whether the collaborative search driving BMm~\eqref{eq:bmm}
reached $p = 0$ at the base seed, and in how many rounds; its successes over $k$ seeds; the same
driving DHNm~\eqref{eq:dhnm}; and successes over $20$ seeds for a single BMm run with no population at
all. That last run is one annealed pass of~\eqref{eq:bmm}, $400$ sweeps at $T = 3.0\cdot0.99^{t}$ from
the same base seed, and is the one configuration here not covered by \tabref{tab:params}.
Backtracking found a completion on every row.}\label{tab:results}
\end{table}
...
instance with givens. The collaborative search reaches a completion on ten of the eleven rows under
BMm and on all eleven under DHNm. Here the collaborative element is not a refinement but the
...
and DHNm succeeds on one of five, and nothing larger was run. A purpose-built
...
\begin{figure}[t]
\centering
% One instance per row, beside its completion; four boards abreast would run to about 16cm.
\begin{tabular}{@{}c@{\hspace{2.2em}}c@{}}
\begin{qcpic}{0.46cm}
...
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}p{0.34\textwidth} p{0.38\textwidth} l@{}}
\toprule
statement & Lean identifier & file\\
\midrule
\eqref{eq:p}, the canonical form & \lstinline{Problem} & \texttt{Problem.lean}\src{HopfieldNet/QUBO/Problem.lean}\\
$p$ is integer-valued & \lstinline{one_le_penaltyR_of_ne_zero} & \texttt{Minimizers.lean}\src{HopfieldNet/QUBO/Minimizers.lean}\\
\thmref{thm:net}, \eqref{eq:Ep} & \lstinline{zeroOneHamiltonian_eq} & \texttt{Net.lean}\src{HopfieldNet/QUBO/Net.lean}\\
minimizers are the feasible points & \lstinline{energy_eq_min_iff} & \texttt{Minimizers.lean}\\
the gap of $\tfrac12$ & \lstinline{half_le_energy_sub_min} & \texttt{Minimizers.lean}\\
the inner loop is twice the local field & \lstinline{netVec_eq_localField} & \texttt{Refine.lean}\src{HopfieldNet/QUBO/Refine.lean}\\
low-temperature concentration, given a completion & \lstinline{stationary_infeasible_tendsto_zero} & \texttt{Gibbs.lean}\src{HopfieldNet/QUBO/Gibbs.lean}\\
Algorithm~2 & \lstinline{search} & \texttt{Search.lean}\src{HopfieldNet/QUBO/Search.lean}\\
\eqref{eq:dhnm}, \eqref{eq:bmm} & \lstinline{dhnmStep}, \lstinline{bmmStep} & \texttt{Dynamics.lean}\src{HopfieldNet/QUBO/Dynamics.lean}\\
\midrule
\eqref{eq:form} is well formed & \lstinline{problem_wf}, \lstinline{problem_refines} & \texttt{Basic.lean}\src{HopfieldNet/QUBO/Instances/Queens/Basic.lean}\\
\figref{fig:inc}, the row layout & \lstinline{rowRow}, \lstinline{colRow}, \lstinline{diagRow}, \lstinline{antiRow} & \texttt{Basic.lean}\\
\figref{fig:inc}, the degrees $4$, $3$, $1$ & \lstinline{rowsOf} & \texttt{Basic.lean}\\
additive diagonal membership & \lstinline{onSlack} & \texttt{Basic.lean}\\
\thmref{thm:iff}, soundness & \lstinline{decode_isQueens} & \texttt{Basic.lean}\\
\thmref{thm:iff}, completeness & \lstinline{encode_penalty_zero} & \texttt{Basic.lean}\\
\thmref{thm:iff}, the equivalence & \lstinline{exists_zero_iff_queens} & \texttt{Basic.lean}\\
\thmref{thm:spec} & \lstinline{isQueens_iff}, \lstinline{exists_zero_iff_completion} & \texttt{Spec.lean}\src{HopfieldNet/QUBO/Instances/Queens/Spec.lean}\\
agreement with an independent checker & \lstinline{isQueens_eq_naive_four/_five} & \texttt{Spec.lean}\\
\thmref{thm:min}, the equivalence & \lstinline{exists_minEnergy_iff} & \texttt{Converge.lean}\src{HopfieldNet/QUBO/Instances/Queens/Converge.lean}\\
\thmref{thm:min}, every minimizer & \lstinline{isQueens_decode_of_minEnergy} & \texttt{Converge.lean}\\
\figref{fig:blocked}(a), (c) & \lstinline{attacking_no_queens}, \lstinline{blocked4_no_queens} & \texttt{Examples.lean}\src{HopfieldNet/QUBO/Instances/Queens/Examples.lean}\\
\figref{fig:blocked}(b) & \lstinline{blk7_no_queens}, \lstinline{blk7_no_zero} & \texttt{Bench.lean}\src{HopfieldNet/QUBO/Instances/Queens/Bench.lean}\\
\figref{fig:comp}, the two instances & \lstinline{small6}, \lstinline{completion8} & \texttt{Examples.lean}\\
\midrule
the baseline returns only completions & \lstinline{solve_isQueens} & \texttt{Baseline.lean}\src{HopfieldNet/QUBO/Instances/Queens/Baseline.lean}\\
\figref{fig:blocked}(d), (e), and the counts & \lstinline{count} & \texttt{Baseline.lean}\\
\tabref{tab:results}, \tabref{tab:blocked} & \lstinline{table}, \lstinline{tableBlocked} & \texttt{Bench.lean}\\
\tabref{tab:params} & \lstinline{SearchConfig}, \lstinline{benchModel} & \texttt{QUBO/Search.lean}, \texttt{Bench.lean}\\
every board drawn above & the \lstinline{#guard}s of \texttt{Figures.lean} & \texttt{Figures.lean}\src{HopfieldNet/QUBO/Instances/Queens/Figures.lean}\\
the animated widget & \lstinline{animate} & \texttt{Widget.lean}\src{HopfieldNet/QUBO/Instances/Queens/Widget.lean}\\
the interactive widget & \lstinline{board}, \lstinline{queensSolve} & \texttt{Interactive.lean}\src{HopfieldNet/QUBO/Instances/Queens/Interactive.lean}\\
\bottomrule
\end{tabular}
\caption{Each statement of this article, its Lean identifier, and its file. Files in the first block
are under \texttt{HopfieldNet/QUBO/}, those in the second and third under
\texttt{HopfieldNet/QUBO/Instances/Queens/} except where a path is shown; each link resolves to the
file itself.}\label{tab:map}
\end{table}
...
\noindent\textbf{The problem.} What is known about $n$-queens is surveyed by Bell and
Stevens~\citeref{bell2009}. Completing a partial placement is a question of Nauck's from 1850, and its
complexity was settled by Gent, Jefferson and Nightingale~\citeref{gent2017}. How many queens may be
placed before completion can fail is now known fairly precisely: Glock, Munh\'{a} Correia and Sudakov
show that any $n/60$ mutually non-attacking queens can always be completed, and that some placements
of about $n/4$ cannot~\citeref{glock2022}. That bound is asymptotic, so it says nothing about the
$7\times7$ instance of \secref{sec:blocked}, which is what makes a machine-checked refutation of one
small board worth having. Counting the solutions of the unconstrained problem is a separate
line~\citeref{simkin2021}.

\medskip\noindent\textbf{Neural networks for $n$-queens.} The problem has been attacked this way
before, and more effectively. A binary Hopfield-type network for exactly this problem was built by
Ma\'{n}dziuk~\citeref{mandziuk1995}, and as \secref{sec:results} records it reaches sizes the
formulation here does not. The problem was used by M\'{e}rida, Mu\~{n}oz and Ben\'{i}tez as the benchmark
for a multivalued recurrent network~\citeref{merida2001}. Both descend from Hopfield and Tank's
proposal to solve optimization problems by minimizing an energy~\citeref{hopfieldtank1985}, and from
Wilson and Pawley's subsequent finding that doing so is delicate~\citeref{wilson1988}. The solver used
here belongs to the collaborative neurodynamic framework of Che and Wang~\citeref{che2019}, which does
carry a global convergence result --- but for an augmented-Lagrangian model, not for the momentum
recurrences~\eqref{eq:dhnm} and~\eqref{eq:bmm} that~\citeref{liwang2022} iterates. That is exactly the
gap \secref{sec:hnbm} describes and \secref{sec:stop} declines to close.

\medskip\noindent\textbf{QUBO encodings.} Writing a problem in the form~\eqref{eq:p} is a
well-developed craft, with Lucas's catalogue~\citeref{lucas2014ising} and the tutorial of Glover,
Kochenberger and Du~\citeref{glover2019} as standard references. The closest work to
\secref{sec:form} is Codognet's~\citeref{codognet2023}, which compares several QUBO encodings of the
at-most-one constraint \emph{using $n$-queens as its example}, on a quantum annealer and on a
quantum-inspired engine. That is the same constraint on the same problem, and the difference is the
point of this article: there the encodings are compared by measuring how well they solve, and none is
proved to be faithful. The auxiliary-variable encoding of \secref{sec:diff} is one of the options
compared there.

\medskip\noindent\textbf{Correctness of encodings.} That a trustworthy solver fed an untrustworthy
encoding still returns an untrustworthy answer has been taken up for propositional formalisms. CNF
encodings are verified in Lean by Codel, Avigad and Heule, at-most-one and at-most-$k$ among
them~\citeref{codel2023}. Szeider's PBLean brings pseudo-Boolean proof certificates into Lean~4 with
the aim pursued here, the translation being formalized together with its correctness proof so that
theorems about the original problem follow~\citeref{szeider2026}. On the solver side the VeriPB line
produces checkable certificates for pseudo-Boolean and constraint solving~\citeref{bogaerts2022}, and
a verified encoding was adopted by Subercaseaux and Heule precisely because theirs had grown too
intricate to trust~\citeref{subercaseaux2023}. What \secref{sec:cert} adds is the same step for the
form $\lVert \hat Ax - \hat b\rVert_2^2$ rather than for clauses or cutting planes, the target being a
network and the objective its energy.

\medskip\noindent\textbf{Formalizations of this problem.} An algorithm producing an $n$-queens
solution for every $n > 3$ was verified in PVS by Malekpour~\citeref{malekpour2020}. That verifies a
\emph{construction}, for the variant that always has a solution; what is certified in
\secref{sec:cert} is a \emph{reduction}, for the variant that is \textsf{NP}-complete, and it is the
reduction that gives a negative answer its content. More distantly, the Cook--Levin theorem was
mechanised in Coq by G\"{a}her and Kunze~\citeref{gaeher2021}, formalizing polynomial-time reductions in
general rather than one encoding for one solver. Closest in shape is Weber's SAT-based Sudoku
solver~\citeref{weber2006}: a puzzle encoded into a solver formalism and evaluated empirically, with
no proof that the encoding is right. To the authors' knowledge no QUBO or Ising encoding has
previously been proved correct in a proof assistant.
...
%%% ---- added with the Related work section; the nine entries above are unchanged.
\bibitem{bell2009}
